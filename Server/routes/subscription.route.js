...
subscription.put("/:subscriptionType", async (req, res) => {
  try {
    ...
    return res.send({ status: "true", subscription });
  } catch (err) {
    console.error(err);
...
subscription.post("/", async (req, res) => {
  try {
    ...
    if (!name || !duration || !price || !onRenewPercentageDiscount) {
      return res
        .status(400)
        .send({ status: "false", message: "Please provide all details" });
    }
    ...
    if (
      ...
      return res
        .status(400)
        .send({ status: "false", message: "Invalid discount percentage" });
    }

    if (taxPercentage && (taxPercentage < 0 || taxPercentage > 100)) {
      return res
        .status(400)
        .send({ status: "false", message: "Invalid tax percentage" });
    }
    ...
    if (onRenewPercentageDiscount < 0 || onRenewPercentageDiscount > 100) {
      return res
        ...
        .send({
          status: "false",
          message: "Invalid on renew discount percentage",
        });
    }
    ...
    res
      .status(500)
      .send({ status: "false", message: "Error creating subscription" });
  }
});
